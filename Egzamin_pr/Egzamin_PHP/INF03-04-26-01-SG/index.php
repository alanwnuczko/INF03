...
<body>
    ...
    <section id="section1">
        ...
        <?php
            ...
            $query = mysqli_query($conn, "SELECT id, imie, nazwisko FROM maturzysta WHERE szkola = 'T3' ORDER BY nazwisko ASC");
            while($row = mysqli_fetch_assoc($query)){
                echo"<a href='wynik.php?id={$row['id']}&imie={$row['imie']}&nazwisko={$row['nazwisko']}'>{$row['id']}. {$row['imie']} {$row['nazwisko']}</a>";
            }
        ?>
    </section>
    <section id="section2">
